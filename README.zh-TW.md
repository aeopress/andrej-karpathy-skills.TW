...
**這是整份 README 最重要的 section。** 我們的 [A/B 實證](./EXPERIMENT.md)（2026 年 5 月）發現下面那三條 reminder 在 Opus 4.7 上沒有可測量效應（最區分維度 N=10 per cell、Fisher exact p=1.00）。但這節的 user-side framing 與 model 無關——不論對面是哪個 LLM、它都會改變你拿回來的東西。
...
三條 reminder，與 [`CLAUDE.md`](./CLAUDE.md) 完全一致。保留是因為成本低、不同 model 或長 context 上或許有用；但在 Opus 4.7 上實證邊際效應不顯著（見 [`EXPERIMENT.md`](./EXPERIMENT.md)）。
